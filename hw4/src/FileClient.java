...
public class FileClient extends UnicastRemoteObject implements ClientInterface {
	...
	/**
	 * Is a constructor that connects to a given DFS server, creates a cache file
	 * entry and sets up the standard input.
	 *
	...
	/**
	 * Is the main loop where the client program reads a file name and its acces
	 * mode from a client, downloads the file if it is not cached, and finally opens
	 * it with emacs or vim.
	 */
	public void loop() {
	...
	/**
	 * Is an RMI function called by the DFS server to invalidate the current cached
	 * file.
	 *
	...
	/**
	 * Is an RMI function called by the DFS server to write back the current cached
	 * file.
	 *
	...
	/**
	 * Is the main function that receives a DFS server's ip name and port#. It then
	 * instantiates a DFS client object and calls the loop( ) function where it
	 * repeats downloading and uploading a file requested by the client user.
	 * 
	 * @param args the array that includes server_ip in its 0th element, the server
	 *             port in the 1st element, and the e option in the 2nd element. If
	 *             e is added, emacs is poppoed out for editing a file. Otherwise,
	 *             vim is invoked.
	 */
	...
	/**
	 * Starts an RMI registry in background, which relieves a user from manually
	 * starting the registry and thus prevents her/him from forgetting its
	 * termination upon a logout.
	 */
	...
	/**
	 * Is a background thread that uploads the cached file back to the DFS server in
	 * response to its writeback request.
	 */
	...
	/**
	 * Is FileClient's private File class. The file client has only one file cache.
	 * Therefore this class instantiates only one object. It has all necessary file
	 * caching logics: invalidate( ), writeback( ), hit( ), download( ), upload( ),
	 * and launchEditor( )
	 */
	private class File {
		...
		public synchronized boolean isStateWriteOwned() {
			System.out.println("name = " + name + " state = " + state + " ownership = " + ownership);
		...
		/**
		 * If the cached file is in readshared, it will be invalidated. Otherwise, the
		 * file is owned and thus can't be invalidated.
		 *
		...
		public synchronized boolean invalidate() {
			if (state == state_readshared) {
				state = state_invalid;
				System.out.println("file( " + name + ") invalidated...state " + state);
				return true;
			} else
				return false;
		}
		...
		/**
		 * If the cached file is in writeowned, it will transit to readshared, so that
		 * the cached file can be uploaded to the server later.
		 *
		 ...
		 *
		 * @return true if a given file is currently cached with a given mode r or w.
		 */
		...
		/**
		 * Downloads file contents of a given filename from the server with mode (r/w).
		 * 
		...
		public boolean download(String filename, String mode) {
			System.out.println("downloading: " + filename + " with " + mode + " mode");
		...
		/**
		 * Is called internally from launchEditor to execute a Unix command such as
		 * "chmod", "emacs", or "vim".
		 *
		 * @param cmd  a unix command such as "chmod", "emacs", or "vim".
		 * @param arg1 the 1st argument passed to this Unix command. It must not be null
		 * @param arg2 the 2nd argument passed to this Unix command. It must not be
		 *             null. If arg2 is not necessary, it must be ""
		...
		private boolean execUnixCommand(String cmd, String arg1, String arg2) {
			...
			String[] cmdarray = arg2.equals("") ? new String[2] : new String[3];
			cmdarray[0] = cmd;
			cmdarray[1] = arg1;
			if (!arg2.equals(""))
				cmdarray[2] = arg2;
		...
		/**
		 * Is called by the main thread's loop( ) function. This function changes the
		 * file mode appropriately with chmod and thereafter invokes emacs or vim.
		 *
